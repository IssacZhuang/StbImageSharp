...
namespace StbTrueTypeSharp
{
	unsafe partial class StbTrueType
	{
		...
		public static int stbtt__solve_cubic(float a, float b, float c, float* r)
		{
			...
			{
				var u = CRuntime.sqrt(-p / 3);
				var v = CRuntime.acos(-CRuntime.sqrt(-27 / p3) * q / 2) / 3;
				var m = CRuntime.cos(v);
				var n = CRuntime.cos(v - 3.141592f / 2) * 1.732050808f;
				r[0] = s + u * 2 * m;
				r[1] = s - u * (m + n);
				r[2] = s - u * (m - n);
				return 3;
			...
			float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
		{
			...
			var dx = x3 - x0;
			var dy = y3 - y0;
			var longlen = CRuntime.sqrt(dx0 * dx0 + dy0 * dy0) + CRuntime.sqrt(dx1 * dx1 + dy1 * dy1) +
								   CRuntime.sqrt(dx2 * dx2 + dy2 * dy2);
			var shortlen = CRuntime.sqrt(dx * dx + dy * dy);
